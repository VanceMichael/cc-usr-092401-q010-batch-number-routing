...
import { TrendingUp, Target, Users, Search, Loader2, Link2 } from 'lucide-react';
...
const Analysis: React.FC = () => {
  ...
  const handleSearchBatch = (e: React.FormEvent) => {
    e.preventDefault();
    const trimmed = searchBatchNumber.trim();
    if (!trimmed) return;
    // 分享、刷新、站内手工查询统一跳转到唯一规范地址 /trace?batch_number=...
    navigate(buildBatchTracePath(trimmed));
  };
    ...
    <div className="space-y-6">
      ...
      <div className="card bg-ocean-50 border-ocean-200">
        ...
        <p className="mt-3 text-sm text-gray-500 flex items-center gap-1">
          <Link2 size={14} />
          查询会打开唯一规范追溯地址（/trace?batch_number=…），可直接复制分享；
          带斜杠、空格或中文的批次号也能正确识别。
        </p>
